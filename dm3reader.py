...
def getDM3FileInfo( dm3_file, return_array=True):
	'''Extracts useful experiment info from DM3 file and 
	returns image to as a numpy array file if 'return_array' set to 'True'.'''
	...
	info_keys = {
		...
		'x_res' : 'root.ImageList.1.ImageData.Dimensions.0',
		'y_res' : 'root.ImageList.1.ImageData.Dimensions.1',
#		'image_notes': 'root.DocumentObjectList.10.Text' # = Image Notes 		
	...
	if tags:
		if return_array:
			# get image array
			im_size = int( tags[ 'root.ImageList.1.ImageData.Data.Size' ] )
			im_offset = int( tags[ 'root.ImageList.1.ImageData.Data.Offset' ] )
			im_width = int( tags[ 'root.ImageList.1.ImageData.Dimensions.0' ] )
			im_height = int( tags[ 'root.ImageList.1.ImageData.Dimensions.1' ] )
			im_pixel_depth = int( tags[ 'root.ImageList.1.ImageData.PixelDepth' ] )
			
			
			if ( debugLevel > 0 ):
				print "im data in", dm3_file, "starts at", hex( im_offset )
				print "im dimension:", im_width, "x", im_height, "size:", im_size
			
			if ( (im_width*im_height*im_pixel_depth) != im_size ):
			...
			for i in range( im_height ):
				for ii in range( im_width ):
					data_bytes = dm3.read(2)
					im_data = struct.unpack('<h', data_bytes)[0]
					im_array += [int( im_data )]
